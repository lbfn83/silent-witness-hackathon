...
export class Vault {
  ...
  _bindOnboarding() {
    ...
    const langOpts = document.querySelectorAll('.sw-lang-opt');
    const storedLang = localStorage.getItem('_lg') || 'en-US';
    langOpts.forEach((opt) => {
      if (opt.dataset.lang === storedLang) {
      ...
      opt.addEventListener('click', () => {
        langOpts.forEach((o) => o.classList.remove('selected'));
        opt.classList.add('selected');
        localStorage.setItem('_lg', opt.dataset.lang);
      });
    });

    // ── Step 3: PIN input ────────────────────────────
    const pinInput = document.getElementById('onboarding-pin-input');
    const pinError = document.getElementById('onboarding-pin-error');
    const hintEl = document.getElementById('onboarding-return-hint');
    ...
    if (pinInput && hintEl) {
      pinInput.addEventListener('input', () => {
        ...
        // Update preview — show typed code or default
        const display = val.length >= 1 ? val : '1337';
        hintEl.innerHTML = `On the calculator, tap: <strong>${display} =</strong>`;
        if (pinError) pinError.style.display = 'none';
      });
    }

    // ── Finish ───────────────────────────────────────
    document.getElementById('onboarding-ready-btn').addEventListener('click', async () => {
      const val = pinInput ? pinInput.value.replace(/\D/g, '') : '';
      ...
      if (val.length >= 3) {
        // Store hash via Calculator static — raw PIN never stored
        const { Calculator } = await import('./calculator.js');
        await Calculator.storePin(val);
      }

      // Persist language
      const sel = document.querySelector('.sw-lang-opt.selected');
      if (sel) localStorage.setItem('_lg', sel.dataset.lang);

      localStorage.setItem('_ob', '1');

      if (localStorage.getItem('sw-model-cached')) {
        this._go('home');
        return;
      }
      ...
      const fillEl   = document.getElementById('ob-model-fill');
      const statusEl = document.getElementById('ob-model-status');

      this.inference.ensureLoaded((pct, msg) => {
        if (fillEl)   fillEl.style.width = pct + '%';
        if (statusEl) statusEl.textContent = msg ?? 'Loading model assets...';
      }).then(() => {
        localStorage.setItem('sw-model-cached', '1');
        this._go('home');
      }).catch(() => {
